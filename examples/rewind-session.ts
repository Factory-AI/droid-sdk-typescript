 ...
 *
 * Usage:
 *   npx tsx examples/rewind-session.ts
 */
...
async function sendAndWait(
  ...
  return new Promise<void>((resolve) => {
    ...
    const unsub = client.onNotification((notification) => {
      ...
      const inner = parsed.data.params.notification;

      if (inner.type === SessionNotificationType.ASSISTANT_TEXT_DELTA) {
        process.stdout.write(inner.textDelta);
      }

      if (inner.type === SessionNotificationType.DROID_WORKING_STATE_CHANGED) {
        if (inner.newState !== DroidWorkingState.Idle) {
          wasActive = true;
        }
        if (inner.newState === DroidWorkingState.Idle && wasActive) {
          unsub();
          resolve();
        }
      }
...
async function main(): Promise<void> {
  ...
  const tempDir = await mkdtemp(join(tmpdir(), 'droid-sdk-rewind-'));
  const transport = new ProcessTransport({ cwd: process.cwd() });
  await transport.connect();
  const client = new DroidClient({ transport });
  const rewindTestFile = join(tempDir, 'rewind-test.txt');
  ...
  try {
    ...
    client.setPermissionHandler((params) => {
      const onlyAllowedCreate = params.toolUses.every(
        (item) =>
          ...
          item.details.filePath === rewindTestFile
      );
    ...
    console.log(
      `=== Turn 1: "hello" (msgId: ${userMsgId1.slice(0, 12)}...) ===`
    );
    ...
    await sendAndWait(
      client,
      `Create a file called ${rewindTestFile} with the content ` +
    ...
    try {
      ...
      console.log(
        `  Created files to delete: ${rewindInfo.createdFiles.length}`
      );
