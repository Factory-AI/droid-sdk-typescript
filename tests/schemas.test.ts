...
describe("enums", () => {
  ...
  it("DroidClientMethod has all 3 methods", () => {
    expect(DroidClientMethod.SESSION_NOTIFICATION).toBe("droid.session_notification");
    expect(DroidClientMethod.REQUEST_PERMISSION).toBe("droid.request_permission");
  ...
  it("DroidWorkingState has correct values", () => {
    ...
    expect(DroidWorkingState.StreamingAssistantMessage).toBe("streaming_assistant_message");
...
describe("shared JSON-RPC schemas", () => {
  ...
  it("JsonRpcEnvelopeSchema rejects wrong jsonrpc version", () => {
    expect(() => JsonRpcEnvelopeSchema.parse({ ...envelope, jsonrpc: "1.0" })).toThrow();
  });
  ...
  it("TraceContextMetaSchema parses optional fields", () => {
    ...
    expect(TraceContextMetaSchema.parse(meta).traceparent).toBe("00-abc-def-01");
    expect(TraceContextMetaSchema.parse({}).traceparent).toBeUndefined();
  });

  it("JsonRpcErrorSchema parses error with data", () => {
    const err = { code: -32004, message: "Not found", data: { sessionId: "s1" } };
...
describe("message content block schemas", () => {
  ...
  it("TextBlockSchema rejects wrong type", () => {
    expect(() => TextBlockSchema.parse({ type: "image", text: "hi" })).toThrow();
  });
  ...
  it("ThinkingBlockSchema parses valid thinking block", () => {
    const block = { type: "thinking", signature: "sig", thinking: "I think..." };
    expect(ThinkingBlockSchema.parse(block).thinking).toBe("I think...");
  });

  it("ToolUseBlockSchema parses valid tool use block", () => {
    const block = { type: "tool_use", id: "tu-1", input: { key: "val" }, name: "Read" };
  ...
  it("DocumentSourceSchema rejects extra fields (strict)", () => {
    const doc = { type: "base64", mediaType: "application/pdf", data: "abc", extra: "field" };
...
describe("MCP schemas", () => {
  ...
  it("McpServerStatusInfoSchema rejects invalid status", () => {
    const info = { name: "srv", status: "invalid_status", source: "user", isManaged: false };
  ...
  it("ToolConfirmationListItemSchema parses valid item", () => {
    ...
    expect(ToolConfirmationListItemSchema.parse(item).value).toBe("proceed_once");
  });
});
...
describe("mission schemas", () => {
  ...
  it("ProgressLogEntrySchema parses mission_accepted entry", () => {
    const entry = { type: "mission_accepted", timestamp: "2024-01-01T00:00:00Z", title: "My Mission" };
  ...
  it("HandoffSchema parses valid handoff", () => {
    const handoff = {
      ...
      whatWasLeftUndone: "",
      verification: { commandsRun: [{ command: "npm test", exitCode: 0, observation: "pass" }] },
      tests: { added: [], coverage: "80%" },
      discoveredIssues: [],
    };
    expect(HandoffSchema.parse(handoff).whatWasImplemented).toBe("Login feature");
  });
});
...
describe("client request params schemas", () => {
  it("InitializeSessionRequestParams parses valid input", () => {
    ...
    expect(InitializeSessionRequestParamsSchema.parse(params).machineId).toBe("m1");
  });

  it("InitializeSessionRequestParams rejects missing required fields", () => {
    expect(() => InitializeSessionRequestParamsSchema.parse({ machineId: "m1" })).toThrow();
  });

  it("LoadSessionRequestParams parses valid input", () => {
    const params = { sessionId: "s-123" };
    expect(LoadSessionRequestParamsSchema.parse(params).sessionId).toBe("s-123");
  });
  ...
  it("KillWorkerSessionRequestParams parses valid input", () => {
    ...
    expect(KillWorkerSessionRequestParamsSchema.parse(params).workerSessionId).toBe("ws-1");
  });
  ...
  it("AuthenticateMcpServerRequestParams parses valid input", () => {
    ...
    expect(AuthenticateMcpServerRequestParamsSchema.parse(params).serverName).toBe("srv");
  });

  it("CancelMcpAuthRequestParams parses valid input", () => {
    const params = { serverName: "srv" };
    expect(CancelMcpAuthRequestParamsSchema.parse(params).serverName).toBe("srv");
  });

  it("ClearMcpAuthRequestParams parses valid input", () => {
    const params = { serverName: "srv" };
    expect(ClearMcpAuthRequestParamsSchema.parse(params).serverName).toBe("srv");
  });

  it("SubmitMcpAuthCodeRequestParams parses valid input", () => {
    const params = { serverName: "srv", code: "abc123", state: "state-token" };
    expect(SubmitMcpAuthCodeRequestParamsSchema.parse(params).code).toBe("abc123");
  });

  it("AddMcpServerRequestParams parses stdio server", () => {
    const params = { name: "my-srv", type: "stdio", command: "node", args: ["srv.js"] };
  ...
  it("RemoveMcpServerRequestParams parses valid input", () => {
    ...
    expect(RemoveMcpServerRequestParamsSchema.parse(params).serverName).toBe("srv");
  });
  ...
  it("ToggleMcpToolRequestParams parses valid input", () => {
    ...
    expect(ToggleMcpToolRequestParamsSchema.parse(params).toolName).toBe("read_file");
  });
  ...
  it("SubmitBugReportRequestParams parses valid input", () => {
    ...
    expect(SubmitBugReportRequestParamsSchema.parse(params).userComment).toBe("Something broke");
  });

  it("strict request schemas reject extra fields", () => {
    expect(() =>
      InitializeSessionRequestParamsSchema.parse({ machineId: "m1", cwd: "/tmp", extra: true }),
    ).toThrow();
  });
});
...
describe("server notification schemas", () => {
  it("AssistantTextDeltaNotificationSchema parses valid notification", () => {
    ...
    expect(AssistantTextDeltaNotificationSchema.parse(n).textDelta).toBe("Hello");
  });
  ...
  it("ThinkingTextDeltaNotificationSchema parses valid notification", () => {
    ...
    expect(ThinkingTextDeltaNotificationSchema.parse(n).textDelta).toBe("thinking...");
  });
  ...
  it("ToolProgressUpdateNotificationSchema parses valid notification", () => {
    ...
    expect(ToolProgressUpdateNotificationSchema.parse(n).toolName).toBe("Execute");
  });
  ...
  it("DroidWorkingStateChangedNotificationSchema parses valid notification", () => {
    ...
    expect(DroidWorkingStateChangedNotificationSchema.parse(n).newState).toBe("idle");
  });
  ...
  it("PermissionResolvedNotificationSchema parses valid notification", () => {
    ...
    expect(PermissionResolvedNotificationSchema.parse(n).selectedOption).toBe("proceed_once");
  });
  ...
  it("SettingsUpdatedNotificationSchema parses valid notification", () => {
    ...
    expect(SettingsUpdatedNotificationSchema.parse(n).settings.modelId).toBe("claude-3");
  });

  it("SessionTitleUpdatedNotificationSchema parses valid notification", () => {
    const n = { type: "session_title_updated", title: "New Title" };
    expect(SessionTitleUpdatedNotificationSchema.parse(n).title).toBe("New Title");
  });

  it("McpStatusChangedNotificationSchema parses valid notification", () => {
    const n = {
      type: "mcp_status_changed",
      servers: [{ name: "srv", status: "connected", source: "user", isManaged: false }],
  ...
  it("SessionTokenUsageChangedNotificationSchema parses valid notification", () => {
    ...
    expect(SessionTokenUsageChangedNotificationSchema.parse(n).sessionId).toBe("s-1");
  });

  it("MissionStateChangedNotificationSchema parses valid notification", () => {
    const n = { type: "mission_state_changed", state: "running" };
    expect(MissionStateChangedNotificationSchema.parse(n).state).toBe("running");
  });
  ...
  it("MissionFeaturesChangedNotificationSchema parses valid notification", () => {
    ...
    expect(MissionFeaturesChangedNotificationSchema.parse(n).features).toHaveLength(1);
  });
  ...
  it("MissionProgressEntryNotificationSchema parses valid notification", () => {
    const n = {
      ...
      progressLog: [
        { type: "mission_accepted", timestamp: "2024-01-01T00:00:00Z", title: "M1" },
      ],
    };
    expect(MissionProgressEntryNotificationSchema.parse(n).progressLog).toHaveLength(1);
  });
  ...
  it("MissionHeartbeatNotificationSchema parses valid notification", () => {
    ...
    expect(MissionHeartbeatNotificationSchema.parse(n).timestamp).toBe("2024-01-01T00:00:00Z");
  });
  ...
  it("MissionWorkerStartedNotificationSchema parses valid notification", () => {
    ...
    expect(MissionWorkerStartedNotificationSchema.parse(n).workerSessionId).toBe("ws-1");
  });
  ...
  it("McpAuthRequiredNotificationSchema parses valid notification", () => {
    ...
    expect(McpAuthRequiredNotificationSchema.parse(n).authUrl).toBe("https://auth.example.com");
  });
...
describe("server→client request schemas", () => {
  ...
  it("RequestPermissionResultSchema parses valid result", () => {
    ...
    expect(RequestPermissionResultSchema.parse(result).selectedOption).toBe("proceed_once");
  });
  ...
  it("AskUserRequestParamsSchema parses valid params", () => {
    const params = {
      toolCallId: "tc-1",
      questions: [
        { index: 1, topic: "Config", question: "Which option?", options: ["A", "B"] },
      ],
    };
