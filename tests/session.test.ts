...
describe('DroidSession', () => {
  ...
  describe('send() (VAL-API-005)', () => {
    ...
    it('calls onMessage for every streamed message and preserves DroidResult', async () => {
      const transport = new InMemoryTransport();
      await transport.connect();

      setupFullResponder(transport, 'sess-send-on-message');

      const session = await createSession({ transport });
      const seenMessages: DroidMessage[] = [];

      const result = await session.send('Stream and aggregate', {
        onMessage(message) {
          seenMessages.push(message);
        },
      });

      expect(seenMessages).toEqual(result.messages);
      expect(seenMessages.map((message) => message.type)).toEqual([
    ...
    it('awaits async onMessage callbacks in order', async () => {
      ...
      setupFullResponder(transport, 'sess-send-on-message-async');

      const session = await createSession({ transport });
      let activeCallbacks = 0;
      let maxConcurrentCallbacks = 0;

      await session.send('Stream with async callbacks', {
        async onMessage() {
          activeCallbacks++;
          maxConcurrentCallbacks = Math.max(
      ...
      expect(maxConcurrentCallbacks).toBe(1);
      expect(activeCallbacks).toBe(0);

      await session.close();
    });

    it('rejects when onMessage throws', async () => {
      const transport = new InMemoryTransport();
      await transport.connect();

      setupFullResponder(transport, 'sess-send-on-message-throws');

      const session = await createSession({ transport });

      await expect(
        session.send('Callback fails', {
          onMessage() {
            throw new Error('onMessage failed');
          },
        })
      ).rejects.toThrow('onMessage failed');

      await session.close();
    });

    it('passes images and files through when using onMessage', async () => {
      const transport = new InMemoryTransport();
      await transport.connect();

      setupFullResponder(transport, 'sess-send-attachments-on-message');

      const session = await createSession({ transport });

      await session.send('Look at these attachments', {
