...
function wireTransport(
  ...
  sessionId: string,
  overrides?: Record<string, (id: string, params: Record<string, unknown>) => void>,
): void {
...
describe("Full query lifecycle (VAL-CROSS-001)", () => {
  it("query() sends initializeSession + addUserMessage, receives streaming notifications, yields correct DroidMessage types, terminates with TurnComplete, and cleans up transport", async () => {
    ...
    const textDeltas = messages.filter((m) => m.type === "assistant_text_delta");
...
describe("Full session lifecycle (VAL-CROSS-002)", () => {
  it("createSession() → session.stream('first') → session.send('second') → session.close()", async () => {
    ...
    expect(streamMessages[streamMessages.length - 1].type).toBe("turn_complete");
    ...
    const addMsgCalls = transport.sentMessages.filter(
      (m) => (m as Record<string, unknown>)["method"] === DroidServerMethod.ADD_USER_MESSAGE,
    );
    expect(addMsgCalls.length).toBe(2);
    expect(
      ((addMsgCalls[0] as Record<string, unknown>)["params"] as Record<string, unknown>)["text"],
    ).toBe("first message");
    expect(
      ((addMsgCalls[1] as Record<string, unknown>)["params"] as Record<string, unknown>)["text"],
    ).toBe("second message");
  ...
  it("multiple turns on same session work correctly with independent state tracking", async () => {
    ...
    if (t1text?.type === "assistant_text_delta") expect(t1text.text).toBe("Turn 1");
    if (t2text?.type === "assistant_text_delta") expect(t2text.text).toBe("Turn 2");
    if (t3text?.type === "assistant_text_delta") expect(t3text.text).toBe("Turn 3");
    ...
    wireTransport(transport, "sess-perm", {
      [DroidServerMethod.ADD_USER_MESSAGE]: (id) => {
        queueMicrotask(() => {
          ...
          transport.injectMessage(
            makeServerRequest("perm-req-001", DroidClientMethod.REQUEST_PERMISSION, {
              toolName: "execute",
              command: "npm test",
    ...
    wireTransport(transport, "sess-perm-session", {
      [DroidServerMethod.ADD_USER_MESSAGE]: (id) => {
        queueMicrotask(() => {
          ...
          // Permission request
          transport.injectMessage(
            makeServerRequest("perm-sess-1", DroidClientMethod.REQUEST_PERMISSION, {
              toolName: "edit",
              path: "/tmp/file.ts",
            }),
          );
...
describe("Ask-user handler integration (VAL-CROSS-004)", () => {
  ...
  it("ask-user handler works through session API", async () => {
    ...
    const session = await createSession({
      ...
      askUserHandler: () => {
        handlerCalled = true;
        return { cancelled: false, answers: [{ questionId: "q1", answer: "Yes" }] };
      },
    });
...
describe("Settings update notification flow (VAL-CROSS-007)", () => {
  it("updateSettings() sent → settings_updated notification arrives → SettingsUpdated message in stream", async () => {
    ...
    const settingsUpdated = messages.filter((m) => m.type === "settings_updated");
    ...
    const { convertNotificationToStreamMessage } = await import("../src/stream.js");
    session.onNotification((notification) => {
      const params = (notification as Record<string, unknown>)["params"] as Record<string, unknown> | undefined;
      const inner = params?.["notification"] as Record<string, unknown> | undefined;
      if (inner) {
        const converted = convertNotificationToStreamMessage(inner as { type: string; [key: string]: unknown });
        if (converted && !Array.isArray(converted) && converted.type === "settings_updated") {
