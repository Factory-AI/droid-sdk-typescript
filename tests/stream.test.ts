...
describe("DroidMessage types", () => {
  ...
  it("DroidMessage union type allows all 22 types", () => {
    ...
    const messages: DroidMessage[] = [
      { type: "assistant_text_delta", messageId: "m1", blockIndex: 0, text: "hi" },
      ...
      { type: "tool_progress", toolUseId: "tu1", toolName: "x", content: "...", update: { type: "status" } },
      { type: "working_state_changed", state: DroidWorkingState.Idle },
      { type: "token_usage_update", inputTokens: 0, outputTokens: 0, cacheReadTokens: 0, cacheWriteTokens: 0, thinkingTokens: 0 },
      { type: "create_message", messageId: "m1", role: "assistant", content: [] },
      { type: "permission_resolved", requestId: "r1", toolUseIds: [], selectedOption: ToolConfirmationOutcome.Cancel },
      { type: "settings_updated", settings: {} },
      { type: "session_title_updated", title: "t" },
      { type: "mcp_status_changed", servers: [], summary: { total: 0, connected: 0, connecting: 0, failed: 0 } },
      ...
      { type: "mcp_auth_required", serverName: "s", authUrl: "u", message: "m", state: "p" },
      { type: "mcp_auth_completed", serverName: "s", outcome: McpAuthOutcome.Success, message: "m" },
...
describe("convertNotificationToStreamMessage", () => {
  ...
  describe("tool_result", () => {
    it("converts string content", () => {
      const notification = makeNotification(
        ...
        { messageId: "msg-1", toolUseId: "tu-1", content: "result text", isError: false },
      );
      const result = convertNotificationToStreamMessage(notification) as ToolResult;
    ...
    it("sets isError from notification", () => {
      const notification = makeNotification(
        SessionNotificationType.TOOL_RESULT,
        { messageId: "msg-1", toolUseId: "tu-1", content: "error", isError: true },
      );
      const result = convertNotificationToStreamMessage(notification) as ToolResult;
  ...
  describe("tool_progress_update", () => {
    it("converts with text field", () => {
      ...
      const result = convertNotificationToStreamMessage(notification) as ToolProgress;
    ...
    it("includes the full update object", () => {
      const update = { type: "tool_call" as const, text: "running", toolName: "exec" };
      const notification = makeNotification(
        SessionNotificationType.TOOL_PROGRESS_UPDATE,
  ...
  describe("droid_working_state_changed", () => {
    it("converts to WorkingStateChanged", () => {
      ...
      const result = convertNotificationToStreamMessage(notification) as WorkingStateChanged;
  ...
  describe("session_token_usage_changed", () => {
    it("converts to TokenUsageUpdate with field mapping", () => {
      ...
      const result = convertNotificationToStreamMessage(notification) as TokenUsageUpdate;
  ...
  describe("error", () => {
    it("converts to ErrorEvent", () => {
      ...
      const result = convertNotificationToStreamMessage(notification) as ErrorEvent;
  ...
  describe("permission_resolved", () => {
    it("converts to PermissionResolved", () => {
      ...
      const result = convertNotificationToStreamMessage(notification) as PermissionResolved;
  ...
  describe("settings_updated", () => {
    it("converts to SettingsUpdated", () => {
      ...
      const result = convertNotificationToStreamMessage(notification) as SettingsUpdated;
  ...
  describe("session_title_updated", () => {
    it("converts to SessionTitleUpdated", () => {
      ...
      const result = convertNotificationToStreamMessage(notification) as SessionTitleUpdated;
  ...
  describe("mcp_status_changed", () => {
    it("converts to McpStatusChanged", () => {
      ...
      const result = convertNotificationToStreamMessage(notification) as McpStatusChanged;
  ...
  describe("mission_state_changed", () => {
    it("converts to MissionStateChanged", () => {
      ...
      const result = convertNotificationToStreamMessage(notification) as MissionStateChanged;
  ...
  describe("mission_features_changed", () => {
    it("converts to MissionFeaturesChanged", () => {
      ...
      const result = convertNotificationToStreamMessage(notification) as MissionFeaturesChanged;
  ...
  describe("mission_progress_entry", () => {
    it("converts to MissionProgressEntry", () => {
      const progressLog = [
        { type: "mission_accepted", timestamp: "2025-01-01T00:00:00Z", title: "Test" },
      ];
      const notification = makeNotification(
        SessionNotificationType.MISSION_PROGRESS_ENTRY,
        { progressLog },
      );
      const result = convertNotificationToStreamMessage(notification) as MissionProgressEntry;
  ...
  describe("mission_heartbeat", () => {
    it("converts to MissionHeartbeat", () => {
      ...
      const result = convertNotificationToStreamMessage(notification) as MissionHeartbeat;
  ...
  describe("mission_worker_started", () => {
    it("converts to MissionWorkerStarted", () => {
      ...
      const result = convertNotificationToStreamMessage(notification) as MissionWorkerStarted;
  ...
  describe("mission_worker_completed", () => {
    it("converts to MissionWorkerCompleted", () => {
      ...
      const result = convertNotificationToStreamMessage(notification) as MissionWorkerCompleted;
  ...
  describe("mcp_auth_required", () => {
    it("converts to McpAuthRequired", () => {
      ...
      const result = convertNotificationToStreamMessage(notification) as McpAuthRequired;
  ...
  describe("mcp_auth_completed", () => {
    it("converts to McpAuthCompleted", () => {
      ...
      const result = convertNotificationToStreamMessage(notification) as McpAuthCompleted;
  ...
  describe("unknown notification type", () => {
    it("returns null for unknown types", () => {
      ...
      const notification = makeNotification("completely_unknown_type", { data: "something" });
      const result = convertNotificationToStreamMessage(notification);
    ...
    it("logs a warning for unknown types", () => {
      const warnSpy = vi.spyOn(console, "warn").mockImplementation(() => {});
      const notification = makeNotification("completely_unknown_type", { data: "something" });
      convertNotificationToStreamMessage(notification);
      expect(warnSpy).toHaveBeenCalledWith(
        "[droid-sdk] Unknown notification type: completely_unknown_type",
      );
      warnSpy.mockRestore();
    });
  ...
  describe("all 20 notification types are handled", () => {
    ...
    it("every notification type returns a non-null result (with valid payloads)", () => {
      ...
      const payloads: Record<string, Record<string, unknown>> = {
        [SessionNotificationType.ASSISTANT_TEXT_DELTA]: { messageId: "m", blockIndex: 0, textDelta: "t" },
        [SessionNotificationType.THINKING_TEXT_DELTA]: { messageId: "m", blockIndex: 0, textDelta: "t" },
        [SessionNotificationType.TOOL_RESULT]: { messageId: "m", toolUseId: "tu", content: "c", isError: false },
        [SessionNotificationType.TOOL_PROGRESS_UPDATE]: { toolUseId: "tu", toolName: "n", update: { type: "status", text: "t" } },
        [SessionNotificationType.DROID_WORKING_STATE_CHANGED]: { newState: DroidWorkingState.Idle },
        [SessionNotificationType.SESSION_TOKEN_USAGE_CHANGED]: {
          sessionId: "s",
          tokenUsage: { inputTokens: 0, outputTokens: 0, cacheReadTokens: 0, cacheCreationTokens: 0, thinkingTokens: 0 },
        },
        [SessionNotificationType.CREATE_MESSAGE]: { message: { id: "m", role: "assistant", content: [] } },
        [SessionNotificationType.ERROR]: { message: "err", errorType: "Error", timestamp: "t" },
        [SessionNotificationType.PERMISSION_RESOLVED]: { requestId: "r", toolUseIds: [], selectedOption: "cancel" },
        [SessionNotificationType.SETTINGS_UPDATED]: { settings: {} },
        [SessionNotificationType.SESSION_TITLE_UPDATED]: { title: "t" },
        [SessionNotificationType.MCP_STATUS_CHANGED]: { servers: [], summary: { total: 0, connected: 0, connecting: 0, failed: 0 } },
        [SessionNotificationType.MISSION_STATE_CHANGED]: { state: MissionState.Running },
        ...
        [SessionNotificationType.MISSION_WORKER_STARTED]: { workerSessionId: "ws" },
        [SessionNotificationType.MISSION_WORKER_COMPLETED]: { workerSessionId: "ws", exitCode: 0 },
        [SessionNotificationType.MCP_AUTH_REQUIRED]: { serverName: "s", authUrl: "u", message: "m", state: "p" },
        [SessionNotificationType.MCP_AUTH_COMPLETED]: { serverName: "s", outcome: McpAuthOutcome.Success, message: "m" },
      };
      ...
      for (const notifType of allNotificationTypes) {
        ...
        const result = convertNotificationToStreamMessage(notification);
        expect(result, `Converter returned null for ${notifType}`).not.toBeNull();
      }
...
describe("StreamStateTracker", () => {
  ...
  describe("multi-turn tracking independence", () => {
    ...
    it("simulates multi-turn session with reset between turns", () => {
      ...
      expect((turn2Result[0] as TurnComplete).tokenUsage!.inputTokens).toBe(200);
    });
  });
