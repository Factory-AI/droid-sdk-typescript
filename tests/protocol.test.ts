...
describe('ProtocolEngine', () => {
  ...
  describe('server→client request handling (VAL-PROTOCOL-004)', () => {
    ...
    describe('hook requests', () => {
      it('invokes registered hook handler and sends response', async () => {
        engine.setHookHandler((params) => ({
          results: [
            {
              exitCode: 0,
              stdout: '',
              stderr: '',
              systemMessage: `handled ${params.eventName}`,
            },
          ],
        }));

        transport.injectMessage(
          makeServerRequest('hook-1', DroidClientMethod.EXECUTE_HOOKS, {
          ...
          results: [
            {
              ...
              stdout: '',
              stderr: '',
              systemMessage: 'handled PreToolUse',
            },
          ],
        });
      });

      it('returns empty hook results when no hook handler is registered', async () => {
        transport.injectMessage(
          makeServerRequest('hook-2', DroidClientMethod.EXECUTE_HOOKS, {
      ...
      it('sends error response when hook handler throws', async () => {
        engine.setHookHandler(() => {
          throw new Error('hook failure');
        });

        transport.injectMessage(
          makeServerRequest('hook-3', DroidClientMethod.EXECUTE_HOOKS, {
            eventName: 'Stop',
            input: {
        ...
        const error = response['error'] as Record<string, unknown>;
        expect(error['code']).toBe(JsonRpcErrorCode.INTERNAL_ERROR);
        expect(error['message']).toBe('Failed to handle hook request');
      });
    });
  });
