...
async function main() {
  // Reset cached daemon state so we spawn a fresh one
  ...
  console.log('=== DAEMON MODE MCP TOOL TEST (droid-dev) ===\n');
  console.log('Connecting to daemon (will spawn droid-dev)...');

  const connection = await connectDaemon();
  console.log('Connected. Creating session with MCP server...');
