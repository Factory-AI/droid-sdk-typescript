/**
 * @fileoverview Require vi.mock() to use absolute paths (no relative paths)
...
module.exports = {
  meta: {
    type: 'problem',
    docs: {
      description: 'Enforce that vi.mock() uses absolute paths only',
      category: 'Testing',
      recommended: true,
    },
    messages: {
      relativePathInViMock:
        'vi.mock() must use absolute paths. Use an absolute path instead of "{{path}}"',
      requireInViMock:
        'vi.mock() should not use require.resolve(). Use the absolute path directly',
    },
    fixable: null,
  ...
  create(context) {
    ...
    /**
     * Check whether a node is vi.doMock() or vi.unmock().
     * @param {Object} node - The AST node
     * @returns {boolean} True if it is vi.doMock()/vi.unmock()/vi.dontMock()
     */
    function isViMockVariant(node) {
      return (
        node.type === 'CallExpression' &&
        node.callee.type === 'MemberExpression' &&
        node.callee.object.type === 'Identifier' &&
    ...
    return {
      CallExpression(node) {
        // Handle vi.mock(), vi.doMock(), vi.unmock(), vi.dontMock()
        if (!isViMockCall(node) && !isViMockVariant(node)) {
          return;
        }
        ...
        if (firstArg.type === 'Literal' && typeof firstArg.value === 'string') {
          ...
          if (isRelativePath(modulePath)) {
            context.report({
              node: firstArg,
              messageId: 'relativePathInViMock',
              data: {
                path: modulePath,
              },
            });
        ...
        // Disallow require.resolve() in vi.mock
        if (
